...
MeasureA4Service::getContours(const cv::Mat &image, std::pair<int, int> threshold, int minArea = 1000,
    ...
    //cannyEdgeDetection(gray, gray, 3, 150, 100);
...
void MeasureA4Service::processImage(const cv::Mat &image) {
    // get A4 paper
    auto contours = getContours(image, std::pair<int, int>(100, 100), 50000, 4);
    if (contours.empty())
        return;
    ...
    // measure rectangle
    contours = getContours(image, std::pair<int, int>(50, 50), 2000, 4, true);
    if (contours.empty())
        return;
