...
private slots:
	...
	void on_btnTestImage_2_clicked();
private:
	int RunTestImage();
	cv::Mat generateQRCode(const std::string& text, int width, int height);
	cv::Mat generateDataMatrix(const std::string& text, int width, int height);
	bool printToSpecificPrinter(const QImage& image, const QString& printerName);
	int RunToolPro();
	int RunToolProDM();
	bool InitDetector();
	bool InitDetectorDM();
	QStringList frmQRcodeIdentify::GetPrinterList();
	bool PrintImageToPrinter(const cv::Mat& image, const QString& printerName);
