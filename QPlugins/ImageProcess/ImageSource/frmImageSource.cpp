	...
	: Toolnterface(toolName, toolBase, parent)
{
	...
	imgIndex = 0;
	connect(this, &frmImageSource::sig_ShowBlockageWarning, this, &frmImageSource::showBlockageWarning);
	connect(this, &frmImageSource::sig_HideBlockageWarning, this, &frmImageSource::hideBlockageWarning);
}
	...
	case 2:
			...
			if (cam_state == 1)
			{
				...
				for (int p = 0; p < keys.length(); p++)
				{
					...
					else if (gvariable.camera_variable_link.value(key).camera_type == "USBCamera")
					{
						//USB相机取像
						QCamera* m_camera = gvariable.camera_variable_link.value(key).usb_camera;
...
void frmImageSource::hideBlockageWarning()
{
	...
	foreach(QWidget * widget, QApplication::topLevelWidgets()) {
		...
		if (label && label->text() == "检测到您遮挡摄像头行为异常") {
			label->close();
			delete label;
			break;
		}
...
void frmImageSource::showBlockageWarning()
{
	// 创建一个QLabel作为窗口
	QLabel* label = new QLabel("检测到您遮挡摄像头行为异常");
...
//检测相机是否被遮挡
bool frmImageSource::isCameraBlocked(cv::Mat& frame)
{
	if (frame.empty()) {
		return false;
	}
	...
	double brightnessStdDev = stddev[0];
	...
	// 条件2: 亮度标准差很小（图像很均匀，可能被遮挡）
	else if (brightnessStdDev < 10) {
		isBlocked = true;
	}
	// 条件3: 运动变化很小（图像静止）
