...
void frmLink::slot_ListClick(QModelIndex index)
{
	...
	toolNames << "全局变量" << "获取图像" << "图像显示" << "导出图像" << "预处理" << "图像拼接" << "图像修复" << "图像细化" << "图像翻转" << "图像旋转" << "透视变换" << "裁切图像" << "创建ROI" << "条形码识别" << "斑点分析" << "二维码识别"
	    ...
	    << "线圆交点" << "点+点" << "点+线" << "线线交点" << "查找圆缺角" << "边缘宽度测量"	<< "拟合平面" << "扩展库" << "跳转语句" << "判断语句" << "结束语句" << "脚本编辑" << "TCP/IP服务器" << "TCP/IP客户端" << "PLC通信" << "串口通信" << "通用I/O" << "延时" << "导出CSV" << "YoloV" << "OCR" << "二维码生成" << "自动打印" << "车牌识别";
	//清空Table
...
QString frmLink::GetIconName(QString Name)
{
	...
	if (Name.contains("车牌识别")) IconName = ":/res/ico/plate.png";
